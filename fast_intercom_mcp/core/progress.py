...
class ProgressBroadcaster:
    ...
    def start_operation(self, operation_type: str, description: str, estimated_items: Optional[int] = None):
    ...
    def update_progress(self, message: str, current: Optional[int] = None, total: Optional[int] = None, **metadata):
        """Update progress for the current operation."""
        event_type = ProgressType.SYNC_PROGRESS if self._current_operation and "sync" in self._current_operation.lower() else ProgressType.INFO
    ...
    def complete_operation(self, message: str, **metadata):
        ...
        event_type = ProgressType.SYNC_COMPLETE if self._current_operation and "sync" in self._current_operation.lower() else ProgressType.INFO
    ...
    def broadcast(self, event: ProgressEvent):
        ...
        if self.console_enabled:
            ...
            should_update = (current_time - self._last_console_update) >= self._console_update_interval
            
            # Always show start/complete events immediately
            force_update = event.event_type in [ProgressType.SYNC_START, ProgressType.SYNC_COMPLETE, ProgressType.ERROR]
    ...
    def _update_console(self, event: ProgressEvent):
        ...
        if event.event_type in [ProgressType.SYNC_PROGRESS, ProgressType.TEST_PROGRESS] and event.percentage:
    ...
    def _update_status_file(self, status: str, operation: Optional[str], description: str):
...
_global_broadcaster: Optional[ProgressBroadcaster] = None


def setup_global_progress_broadcaster(log_dir: Optional[str] = None, session_id: Optional[str] = None) -> ProgressBroadcaster:
...
def get_progress_broadcaster() -> Optional[ProgressBroadcaster]:
...
def start_operation(operation_type: str, description: str, estimated_items: Optional[int] = None):
...
def update_progress(message: str, current: Optional[int] = None, total: Optional[int] = None, **metadata):
