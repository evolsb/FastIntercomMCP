...
def test_environment():
    ...
    cli_test = run_timed_test(
        'python3 -m fast_intercom_mcp --help',
        "CLI availability test"
    )
    
    # Check token availability
    token_available = bool(os.environ.get('INTERCOM_ACCESS_TOKEN'))
    print(f"{'✅' if token_available else '❌'} API token {'available' if token_available else 'missing'}")
...
def test_server_startup():
    ...
    server_help_test = run_timed_test(
        'timeout 10s python3 -m fast_intercom_mcp server --help',
        "Server help command"
    )
...
def generate_performance_report(test_results):
    ...
    report = {
        ...
        'summary': {
            'overall_success': all(result.get('success', False) for result in test_results.values() if isinstance(result, dict) and 'success' in result),
        }
    }
    ...
    if 'integration_test' in test_results and 'db_metrics' in test_results:
        ...
        if integration['success'] and db_metrics['conversations'] > 0:
            ...
            rating = 'EXCELLENT' if score >= 5 else 'GOOD' if score >= 3 else 'FAIR' if score >= 1 else 'NEEDS_IMPROVEMENT'
...
def main():
    ...
    if test_results.get('db_metrics', {}).get('conversations', 0) > 0:
        ...
        if 'performance_metrics' in report:
            ...
            print(f"  • Avg conversation size: {perf['database_efficiency_kb_per_conversation']:.1f}KB")
