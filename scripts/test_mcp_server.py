...
class MCPServerTester:
    ...
    async def test_tool(self, tool_name: str, arguments: Dict[str, Any], 
                       description: str, expected_error: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def test_list_tools(self) -> bool:
        ...
        try:
            ...
            for tool in tools:
                if "name" not in tool:
                    self.log_error(f"Tool missing 'name' field")
                    self.failed_tests += 1
                    return False
        ...
        try:
            ...
            if os.getenv("FASTINTERCOM_TEST_FORCE_SYNC", "false").lower() == "true":
                result = await self.test_tool(
                    "force_sync",
                    {"days": 1},
                    "Force sync last 1 day"
                )
                self.results.append(result)
            else:
                self.log_warning("Skipping force_sync test (set FASTINTERCOM_TEST_FORCE_SYNC=true to enable)")
        ...
        if self.failed_tests > 0:
            ...
            for result in self.results:
                if result.get("status") in ["FAILED", "ERROR"]:
                    print(f"  - {result['tool']}: {result.get('error', result.get('validation_errors', 'Unknown error'))}")
                    
        # Log summary for automated parsing
        logger.info(f"MCP_TEST_SUMMARY: total={self.total_tests}, passed={self.passed_tests}, "
