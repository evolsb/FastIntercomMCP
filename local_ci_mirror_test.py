...
async def run_quick_integration_test(test_dir, sync_days=1):
    ...
    try:
        ...
        print(
            f"🔄 Quick sync: {sync_days} day(s) of data (limited to 50 conversations for speed)"
        )
        print(
        ...
        print(
            f"🔄 Using sync_period with last 2 hours: {recent_time.strftime('%H:%M')} to {end_date.strftime('%H:%M')}"
        )
