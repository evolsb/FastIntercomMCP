...
class IntegrationTestRunner:
    ...
    async def test_initial_sync(self) -> bool:
        ...
        try:
            ...
            if sync_speed < self.performance_targets['min_sync_speed']:
                logger.warning(f"⚠️  Sync speed ({sync_speed:.1f} conv/sec) below target ({self.performance_targets['min_sync_speed']} conv/sec)")
    ...
    async def test_mcp_server_functionality(self) -> bool:
        ...
        if not DEPENDENCIES_AVAILABLE or not self.db or not self.sync_service or not self.intercom_client:
        ...
        try:
            ...
            if status_time > max_response_time:
                logger.warning(f"⚠️  Status response time ({status_time*1000:.1f}ms) exceeds target ({max_response_time*1000:.1f}ms)")
            if search_time > max_response_time:
                logger.warning(f"⚠️  Search response time ({search_time*1000:.1f}ms) exceeds target ({max_response_time*1000:.1f}ms)")
    ...
    def check_performance_targets(self) -> bool:
        ...
        if sync_speed < self.performance_targets['min_sync_speed']:
            logger.warning(f"⚠️  Sync speed target missed: {sync_speed:.1f} < {self.performance_targets['min_sync_speed']}")
            targets_met = False
        else:
        ...
        response_times = [
            ...
            ('get_conversation', self.performance_metrics.get('get_conversation_time', 0))
        ]
        ...
        for name, time_val in response_times:
            if time_val > max_response_time:
                logger.warning(f"⚠️  {name} response time target missed: {time_val*1000:.1f}ms > {max_response_time*1000:.1f}ms")
                targets_met = False
            else:
    ...
    def print_final_report(self) -> None:
        ...
        if 'conversations_synced' in self.performance_metrics:
            logger.info(f"   Conversations synced: {self.performance_metrics['conversations_synced']:,}")
        ...
        if 'sync_duration' in self.performance_metrics:
            logger.info(f"   Sync duration: {self.performance_metrics['sync_duration']:.1f} seconds")
            
        # Response times
        response_times = [
            ('Status', self.performance_metrics.get('status_response_time', 0)),
            ('Search', self.performance_metrics.get('search_response_time', 0)),
            ('Get Conversation', self.performance_metrics.get('get_conversation_time', 0))
        ]
        ...
        logger.info("")
        all_passed = all(
            result for key, result in self.test_results.items() 
            if isinstance(result, bool)
        )
